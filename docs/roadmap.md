...
1. **~~统一请求层~~（已完成）**：扩展请求统一走 `lib/api.ts` 的 `apiFetch`（自动带 JWT、401 清理登录态、非 2xx 抛错并携带后端 message）；`postWord` 修正对 `success: false` 的判断，失败正确进入离线队列。
...
4. **~~API 文档~~（已完成）**：`docs/api.md` 汇总全部接口的鉴权、请求/响应结构与频控约定；后续如引入 flask-smorest 可再升级为 OpenAPI 自动生成。
