...
# 微信开放平台扫码登录（Chrome 扩展）：一次性登录票据缓存键前缀
# 票据初始值 pending，扫码确认后写入 {user_info, token} JSON，扩展轮询消费
CACHE_WECHAT_LOGIN_TICKET_PREFIX = "BCZ:WECHAT_LOGIN_TICKET"
# 票据有效期 300s，与微信二维码页面有效期对齐
CACHE_WECHAT_LOGIN_TICKET_TIMEOUT = 300
